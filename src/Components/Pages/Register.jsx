...
//     setIsLoading(true);
//     try {
//       const { data } = await axios.post('https://zencia-finalbackend.vercel.app/api/verfiy', {
//         email,
//         otp: otpString,
//       });
//       const { token, redirectPath } = data;
...
//                 disabled={isLoading} />
//                 <label htmlFor="terms">
//                   I agree to the <a href="https://www.zencia.ai/t&c.html">Terms of Service</a> and <a href="https://www.zencia.ai/privacy-policy.html">Privacy Policy</a>
//                 </label>
//               </div>
//               <button
//                 type="submit"
...
function Register({ onSuccess }) {
  ...
  const handleResendOtp = async () => {
    ...
    setSuccessMessage('');
    try {
      const { data } = await axios.post('https://zencia-finalbackend.vercel.app/api/resend', {
        email,
      });
      setSuccessMessage(data.message || 'New OTP sent to your email.');
      setOtp(['', '', '', '', '', '']); // Clear OTP inputs
      otpInputRefs[0].current.focus(); // Focus on first input
    } catch (error) {
      ...
      <div className="register-container">
        ...
        <div className="register-form-section">
          ...
          {!isOtpVisible ? (
            ...
            Already have an account? <a href="/auth/login" className="register-link">Sign In</a>
          </div>
            </form>
