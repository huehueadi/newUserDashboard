  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const { error, paymentIntent } = await stripe.confirmPayment({
        elements,
        confirmParams: {
          return_url: `http://localhost:5173/success?paymentIntentId=${paymentIntentId}`, // Updated to /success
        },
      });
      ...
      if (error) {
        ...
        window.location.href = `http://localhost:5173/success?paymentIntentId=${paymentIntentId}`; // Updated to /success
      } else {
